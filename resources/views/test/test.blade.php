...
<script>
  ...
  function sendImagesToServer(studentId, base64String) {
    ...
    fetch('http://127.0.0.1:8000/Student/add-image/'+studentId, {
...
<script>
  function checkParentGuardian(parentGuardianName, nickname, phoneNumber,password, address) {
    ...
    return fetch('http://127.0.0.1:8000/parent/checkExistence', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
<script>
    function fetchCardRFID(){
        fetch('http://127.0.0.1:8000/rfid/all-data')
            .then(response => response.json())
            .then(data => {
    ...
    function fetchTagRFID(){
        fetch('http://127.0.0.1:8000/rfid/all-data-tag')
            .then(response => response.json())
            .then(data => {
